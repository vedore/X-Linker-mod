...
from src.python.utils import parse_json, get_dataset_abbreviations, prepare_input, calculate_topk_accuracy
...
for i, mention in enumerate(test_input):
    ...
    #Encode query
    query_toks = tokenizer.batch_encode_plus([mention], 
                                        padding="max_length", 
                                        max_length=25, 
                                        truncation=True,
                                        return_tensors="pt")
    ...
    predictions.append([test_annots[i][0], test_annots[i][1], test_annots[i][2], test_annots[i][3], true_label, pred_labels, scores])
    pbar.update(1)
...
predictions_df = pd.DataFrame(predictions, columns= ["doc_id", "start", "end", "text", "code", "codes" , "scores"])
