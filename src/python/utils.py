...
def convert_bioc_xml_2_bioc_json(in_filepath, out_filepath):
    """Converts a BioC XML file to a BioC JSON file"""
...
def convert_bioc_json_2_bioc_xml(in_filepath, out_filepath):
    """Converts a BioC JSON file to a BioC XML file"""
...
def convert_pubtator_2_bioc_json(in_filepath, out_filepath):
    """Converts a PubTator file to a BioC JSON file"""
        ...
        if file[-3:] == "ann":
            ...
            for line in document:
                ...
                if dataset == "craft_ncbi_taxon":
                    #print(line.split("\t"))
                    #['T100', 'NCBITaxon:10088 53936 53940', 'mice\n']
                ...
                doc_annotations.append(
                    (start, end, annotation_text, kb_id))
...
def parse_bioc_json(in_filepath, ent_types=[], dataset=None):
    """Parses a BioC JSON file and returns the annotations. 
    The annotations are the input for an Entity Linking 
    evaluation tool"""
...
def parse_pubtator(dataset, ent_types="", evaluate=True):
    """Parses the Pubtator format of the datasets and returns a list of
    documents IDs or a list of annotations

    Arguments:
        dataset {str} -- Name of the dataset to parse

    Keyword Arguments:
        get_annots {bool} -- If True, returns a list of annotations 
            (default: {False})
        only_doc_ids {bool} -- If True, returns a list of document IDs 
            (default: {False})
    ...
    if dataset == "bc5cdr":
        ...
        ent_type = ["Modifier", "SpecificDisease", "DiseaseClass"]
    ...
    for filename in filenames:
        ...
        with open(f"{data_dir}/{filename}", "r", encoding="utf-8") as pubtator_file:
...
def parse_dataset(
        input_format, dataset=None, data_dir=None, ner_filepath=None, 
        ent_types=None):
    """Parses a dataset into a dictionary of annotations"""
...
def prepare_nlm_chem():
    """Prepares the NLM-Chem dataset for Entity Linking evaluation. Converts
...
def convert_linnaeus_2_bioc_json():
    """Converts the Linnaeus dataset to the BioC JSON format. 
    Create the file test.json in the directory 'data/datasets/linnaeus'
    """
    ...
    for i, filename in enumerate(filenames):
        ...
        out_dict["documents"].append({"id": doc_id, "infons": "", "passages": []})
        text = open(f"{data_dir}/txt/{filename}", "r", encoding="utf-8").read()#.replace("\n", "$")
...
def convert_bc2gn_to_json():
    """Converts the BC2GN dataset to the BioC JSON format.
    Create the file 'test.json' in the directory 'data/datasets/bc2gn'
    """
...
def convert_datasets_to_bioc_json():
    ...
    In each dataset directory, it generates a file named 'test.json'
    """
    ...
    os.system("""sed 's/"type": "Modifier"/"type": "Disease"/g' data/datasets/ncbi_disease/test_tmp.json | sed 's/"type": "SpecificDisease"/"type": "Disease"/g' | sed 's/"type": "DiseaseClass"/"type": "Disease"/g' > data/datasets/ncbi_disease/test_tmp2.json""")
    os.system("""sed 's/"cui":/"identifier":/g' data/datasets/ncbi_disease/test_tmp2.json > data/datasets/ncbi_disease/test.json""")
    ...
    os.system("""awk -F'\t' '{ print $1 "\t" $2 "\t" $3 "\t" $4 "\t" "BIO" "\t" $6 }' data/datasets/med_mentions/corpus_pubtator.txt > data/datasets/med_mentions/corpus_pubtator_type_corrected""")

    annotations = parse_pubtator("med_mentions")
    output_pubtator(annotations,
...
def generate_txt_files(input_format, dataset=None, data_dir=None, 
                      ner_filepath=None, ent_types=None):
    """"Generate text files for each document in given dataset"""

    annotations = parse_dataset(
        input_format, dataset=dataset, data_dir=data_dir, 
        ner_filepath=ner_filepath,ent_types=ent_types)
...
def dataset_stats():
    ...
    annotations = parse_dataset(
            "bioc_json", dataset=None, data_dir=None, ner_filepath= f"data/datasets/{dataset}/test.json", 
            ent_types=ent_type)
...
def create_datasets_pmids_list(email=None):
    """Create a file with PMIDs to ignore since they correspond to documents
    present in evaluation datasets. The goal is to further generate unbiased 
    training data.
    ...
    #BC5CDR
    bc5cdr_json = parse_bioc_json("data/datasets/bc5cdr/test.json")
    bc5cdr_pmids = list(bc5cdr_json.keys())
    pmids_list.extend(bc5cdr_pmids)
    out_info += f"BC5CDR IDs added: {len(bc5cdr_pmids)}\n"

    # Import PMIDs from NCBI Disease
    ncbi_disease_json = parse_bioc_json("data/datasets/ncbi_disease/test.json")
    ncbi_disease_pmids = list(ncbi_disease_json.keys())
    pmids_list.extend(ncbi_disease_pmids)
    out_info += f"NCBI Disease IDs added: {len(ncbi_disease_pmids)}\n"

    # Import PMIDs from BioRED
    biored_json = parse_bioc_json("data/datasets/biored/test.json")
    biored_pmids = list(biored_json.keys())
    ...
    #Output info file
    with open(f"{datasets_dir}/ignore_pmids_info.txt", "w", encoding="utf-8") as out_file2:
...
def generate_kb_mappings(kb):
    """Mappings between knowledge Base names and labels"""
    ...
    if kb == 'medic':
        ...
        col_names = ["DiseaseName", "DiseaseID", "CasRN", "Definition", 
            "ParentIDs", "TreeNumbers", "ParentTreeNumbers", "Synonyms", "pad"]
        ...
        col_names = ["ChemicalName", "ChemicalID", "CasRN", "Definition", 
            "ParentIDs", "TreeNumbers", "ParentTreeNumbers", "Synonyms", "pad"]
        data = pd.read_csv(f'{data_filepath}', names=col_names, sep='\t', skiprows=29)  
        ...
        col_names = ["GeneSymbol", "GeneName", "GeneID", "AltGeneIDs", 
            "Synonyms", "BioGRIDIDs", "PharmGKBIDs", "UniProtIDs"]
        data = pd.read_csv(f'{data_filepath}', names=col_names, sep='\t', skiprows=29)  
...
def add_predicted_kb_identifers(
    ...
    """Adds predicted identifiers to NER output
