"""Evaluate PECOS-EL and X-Linker in datasets"""
import pandas as pd
import os
from argparse import ArgumentParser, BooleanOptionalAction
from src.python.xtransformer.utils import load_model, load_kb_info, process_pecos_preds, apply_pipeline_to_mention
from src.python.utils import get_dataset_abbreviations, prepare_input, calculate_topk_accuracy
...
# Load KB info
#----------------------------------------------------------------------------
id_2_name, index_2_id, synonym_2_id_lower, name_2_id_lower, \
    kb_names, kb_synonyms = load_kb_info(args.kb, inference=True) 
...
predictions_df = pd.DataFrame(output, columns= [
    "doc_id", 
    "start", 
    "end", 
    "text", 
...
if args.ppr:
    ...
    pred_path = f"data/models/evaluation/{args.dataset}_{args.ent_type}.tsv"
    predictions_df.to_csv(pred_path, sep='\t', index=False)
