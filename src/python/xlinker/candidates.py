...
def check_if_related(candidate1, candidate2, kb_edges):
    ...
    This function checks whether there is a direct relationship between
    ...
    if candidate1["url"] in kb_edges and \
            candidate2["url"] in kb_edges[candidate1["url"]]:
        related = True

    if candidate2["url"] in kb_edges and \
            candidate1["url"] in kb_edges[candidate2["url"]]:
        related = True
...
def get_candidates_info(candidates, id_to_info):
    ...
    -------
    dict of str to dict
        A dictionary where each key is a candidate identifier from the `candidates` list
        and each value is a dictionary containing the candidate's information and their
        relationships. The relationships are stored under the key 'relationships'.
    """
    candidates_info = []
    ...
    for cand in candidates:
        ...
        candidates_info.append(
            {"url": cand_kb_id, "name": cand_text,
            "outcount": outcount, "incount": incount, "id": cand_id, 
            "links": [], "score": cand_score})
