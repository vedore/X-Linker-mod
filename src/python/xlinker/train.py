...
parser.add_argument("-model", type=str, default="bert", 
    choices=["bert", "roberta", "biobert", "scibert", "pubmedbert"], help="")
parser.add_argument("-clustering", type=str, default="pifa", 
    choices=["pifa", "pifa_lf"], help="")
...
logging.basicConfig(level=logging.INFO, format=log_format, handlers=[
    ...
    RotatingFileHandler(
        ...
        backupCount=2)  # To write to a rotating file
])
logging.info("\n------------------------------------------")
...
if not os.path.exists(params_filepath):
    logging.info('No train_params.json file found! Copying from data/models/trained/train_params.json')
    os.system(f"cp data/models/trained/train_params.json data/models/trained/{RUN_NAME}/train_params.json")
...
params['train_params']['matcher_params_chain']['batch_gen_workers'] = args.batch_gen_workers
...
parsed_train_data = Preprocessor.load_data_from_file(
    train_filepath,
    label_text_path=f"{KB_DIR}/labels.txt")
...
logging.info(f"Constructed training corpus len={len(X_train)}, training label matrix with shape={Y_train.shape} and nnz={Y_train.nnz}")
...
logging.info(f"Constructed TRAINING feature matrix with shape={X_train_feat.shape} and nnz={X_train_feat.nnz}")
